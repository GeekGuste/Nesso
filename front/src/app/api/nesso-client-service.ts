...
export class Client {
     ...
     * @return Successful Response
     */
    post(body: GetRecipeInput): Observable<GetRecipeOutput> {
        let url_ = this.baseUrl + "/get-recipe/";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<GetRecipeOutput>;
                }
            } else
                return _observableThrow(response_) as any as Observable<GetRecipeOutput>;
        }));
    }

    protected processPost(response: HttpResponseBase): Observable<GetRecipeOutput> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = GetRecipeOutput.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status === 422) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<GetRecipeOutput>(null as any);
    }
}
...
export class GetRecipeInput implements IGetRecipeInput {
    ...
    constructor(data?: IGetRecipeInput) {
        if (data) {
    ...
    static fromJS(data: any): GetRecipeInput {
        data = typeof data === 'object' ? data : {};
        let result = new GetRecipeInput();
        result.init(data);
        return result;
    }
...
export interface IGetRecipeInput {
    ingredients: string[];

    [key: string]: any;
}

export class GetRecipeOutput implements IGetRecipeOutput {
    recipes?: Recipe[];

    [key: string]: any;

    constructor(data?: IGetRecipeOutput) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (this as any)[property] = (data as any)[property];
    ...
    static fromJS(data: any): GetRecipeOutput {
        data = typeof data === 'object' ? data : {};
        let result = new GetRecipeOutput();
        result.init(data);
        return result;
    }
...
export interface IGetRecipeOutput {
    recipes?: Recipe[];
